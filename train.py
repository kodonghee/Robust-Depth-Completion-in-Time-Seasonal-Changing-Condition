...
from models import ResNet # cspn revised
...
def create_data_loaders(args):
    ...
    rgb_traindir = os.path.join('data', args.data, 'rgb_day', 'train')
    dep_traindir = os.path.join('data', args.data, 'depth_day', 'train')
    sky_traindir = os.path.join('data', args.data, 'sky_day', 'train')
    rgb_valdir = os.path.join('data', args.data, 'rgb_day', 'val')
    dep_valdir = os.path.join('data', args.data, 'depth_day', 'val')
    sky_valdir = os.path.join('data', args.data, 'sky_day', 'val')
    train_loader = None
    val_loader = None
    ...
    # create new model
        ...
        if args.arch == 'resnet50':
            ...
            model = ResNet(layers=18, decoder=args.decoder, output_size=train_loader.dataset.output_size,
                in_channels=in_channels, pretrained=args.pretrained)
            # revised cspn
            # import models_cspn as model
            # model = model.resnet18(pretrained = 'store_true',
            #                        cspn_config=cspn_config)

            # params = [p.size() for p in model.parameters()]
            # print(params)
        print("=> model created.")
