...
def parse_command():
    ...
    parser.add_argument('--data', metavar='DATA', default='synthia',
                        choices=data_names,
                        help='dataset: ' + ' | '.join(data_names) + ' (default: synthia)')
    ...
    parser.add_argument('-b', '--batch-size', default=8, type=int, help='mini-batch size (default: 16)')
...
def get_output_directory(args):
    ...
    output_directory = os.path.join('ftest_3', 'test_day')
...
def merge_into_row_with_gt(input, depth_input, depth_target, depth_pred):
    ...
    depth_target_cpu = np.squeeze(depth_target.cpu().numpy())
    depth_pred_cpu = np.squeeze(depth_pred.data.cpu().numpy())
    depth_pred_cpu = np.where(depth_target_cpu == 0, 0, depth_pred_cpu)
